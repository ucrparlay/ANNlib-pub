...
public:
	...
	template<std::ranges::range R, class BinaryOp=std::plus<>>
	static auto scan(const R &r, BinaryOp op={})
	{
		using namespace std::ranges;
		using value_t = range_value_t<R>;

		seq<value_t> res;
		res.reserve(size(r)+1); // reserve for tot
		...
		value_t tot = op(res.back(), r.back());
		return std::pair(res, tot);
	}
};
