...
inline std::pair<parlay::sequence<parlay::sequence<L>>,
                 std::variant<std::unordered_map<L, parlay::sequence<pid_t>>,
                              parlay::sequence<std::pair<L, parlay::sequence<pid_t>>>>>
load_label(const char *file_path, uint32_t max_size = 0, bool ret_pair = true) {
  ...
  // parlay::sequence<parlay::sequence<L>> F(num_points);
  // std::unordered_map<L, parlay::sequence<pid_t>> P;
  // size_t total_labels = 0;
  // const L separater = -1;

  // for (uint32_t i = 0; i < num_points; ++i) {
  //   parlay::sequence<L> node_labels;
  ...
  std::string line;
  parlay::sequence<parlay::sequence<L>> F;
  std::unordered_map<L, parlay::sequence<pid_t>> P;
  ...
  while (std::getline(file, line) && (!max_size || num_points<max_size)) {
    ...
    ++num_points;
    F.push_back(parlay::sort(std::move(labels)));
  }
  ...
  parlay::sequence<std::pair<L, parlay::sequence<pid_t>>> pairs(P.begin(), P.end());

  std::sort(
      pairs.begin(), pairs.end(),
      [&](const std::pair<L, parlay::sequence<pid_t>> &a, const std::pair<L, parlay::sequence<pid_t>> &b) {
