...
void print_stat(const G &g)
{
	...
	size_t cnt_vertex = g.num_nodes(0);
	size_t cnt_degree = g.num_edges(0);
...
void run_test(commandLine parameter) // intend to be pass-by-value manner
{
	...
	const uint32_t m_l = parameter.getOptionIntValue("-ml", 0.36);
	...
	*/
	...
	size_t num_nodes = g.num_nodes(0);
	size_t num_edges = g.num_edges(0);
	size_t max_deg = g.max_deg(0);
