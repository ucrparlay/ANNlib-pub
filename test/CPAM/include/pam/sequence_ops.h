...
struct sequence_ops : Tree {
  ...
  static size_t size_in_bytes(node* a, const F& f) {
    ...
    auto P = utils::fork<size_t>(n >= utils::node_limit,
      [&] () {return size_in_bytes(a->lc, f);},
      [&] () {return size_in_bytes(a->rc, f);});

    total += P.first + P.second;
    total += f(Tree::get_entry(a));
    return total;
  }
  ...
  static std::tuple<size_t, size_t, size_t> node_stats(node* a) {
    ...
    size_t n = Tree::size(a);
    auto P = utils::fork<std::tuple<size_t, size_t, size_t>>(n >= utils::node_limit,
      [&] () {return node_stats(a->lc);},
      [&] () {return node_stats(a->rc);});
  ...
  static bool foreach_cond(node* a, const F& f) {
    ...
    if (foreach_cond(a->lc, f) && f(Tree::get_entry(a))) {
      return foreach_cond(a->rc, f);
    }
    return false;
  }
