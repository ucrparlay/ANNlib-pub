...
public:
  ...
  // api)
  template <class F>
  static void foreach_index_2(const M& m, const F& f, size_t start=0,
			    size_t granularity = utils::node_limit) {
    Tree::foreach_index(m.root, start, f, granularity, true);
  }
  ...
  // apply function f on all entries sequentially. F returns a boolean
  // indicating whether to proceed further or not.
  // experimental: only to test cpam interface
  template <class F, class C>
  static bool foreach_cond_par(const M& m, const F& f, const C& c) {
    return Tree::foreach_cond(m.root, f);
  }
  ...
  void insert(const E& p) {
    auto replace = [] (const V& a, const V& b) {(void)a; return b;};
  ...
  static M multi_delete_sorted(M m, Seq const &SS) {
    return M(Tree::multi_delete_sorted(m.get_root(), SS.begin(), SS.size()));
  }
