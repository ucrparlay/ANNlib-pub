...
public:
  ...
  static size_t size(node* r) { return Tree::size(r); }
  ...
  static void foreach_seq(const M& m, const F& f) {
    Tree::foreach_seq(ptr(m.root, true), f);
  }

  // apply function f on all entries sequentially. F returns a boolean
  // indicating whether to proceed further or not.
  template <class F>
  static bool foreach_cond(const M& m, const F& f) {
    return Tree::foreach_cond(m.root, f);
  }

  // apply function f on all entries sequentially. F returns a boolean
  // indicating whether to proceed further or not.
  template <class F, class C>
  static bool foreach_cond_par(const M& m, const F& f, const C& cond) {
    return Tree::foreach_cond_par(m.root, f, cond);
  }
  ...
  template<class F>
  static M filter(M m, const F& f, size_t granularity=kNodeLimit) {
    return M(Tree::finalize(Tree::filter(m.get_root(), f, granularity))); }
...
//    t.next("union time");
  ...
  static bool multi_update_sorted_inplace(M& m, Seq const &SS, Bin_Op f) {
    return Tree::multi_update_sorted_inplace(m.root, SS.data(), SS.size(), f);
  }
...
//  }
  ...
  static parlay::sequence<V> multi_find_sorted(M& m, Seq const &SS) {
    parlay::sequence<V> ret(SS.size());
    Tree::multi_find_sorted(m.root, SS.begin(), SS.size(), ret.begin(), 0);
    return ret;
  }
  ...
  static M multi_insert_sorted(M m, Seq const &SS, BinOp f) {
    return M(Tree::finalize(Tree::multi_insert_sorted(m.get_root(), SS.begin(), SS.size(), f)));
  }
  ...
  static M multi_insert_sorted_map(M m, Seq &SS, CombineOp combine_op, MapOp map_op) {
    //std::cout << "sorted_map, ref_cnt = " << m.ref_cnt() << std::endl;
    return M(Tree::multi_insert_sorted_map(m.get_root(), SS.begin(), SS.size(), combine_op, map_op));
  }

  // insert multiple keys from an array
  template<class Seq, class CombineOp>
  static M multi_delete_sorted_map(M m, Seq &SS, CombineOp combine_op) {
    return M(Tree::multi_delete_sorted_map(m.get_root(), SS.begin(), SS.size(), combine_op));
  }

  // insert multiple keys from an array
  template<class Seq>
  static M multi_delete_sorted(M m, Seq const &SS) {
    return M(Tree::finalize(Tree::multi_delete_sorted(m.get_root(), SS.begin(), SS.size())));
  }
  ...
  template <class Func>
  void iterate_cond(const Func& f) {
    Tree::iterate_cond(root, f);
  }
...
//
  ...
  maybe_E select(const size_t rank) const {
    auto ret = Tree::select(root, rank);
    return ret;
  }
  ...
  static M subseq(M& a, const size_t left, const size_t right) {
    return M(Tree::subseq(a.root, left, right));}

  // Sequence only
  M append(M& other) {
    return M(Tree::join2_i(ptr(root, true), ptr(other.root, true))); }

  // Sequence Op: returns index of first element equal to e
  std::optional<size_t> find_unsorted(const E& e) {
...
// TODO
  ...
  static M range(M& a, K kl, K kr) {
    return M(Tree::range_2(a.root, kl, kr));
    //return M(Tree::range(ptr(a.root, true), kl, kr));
  }
...
//  }
  ...
  static M map(Ma& a, const F& f) {
    ...
    // Unnecessary ref-cnt hits: return M(Tree::template map<typename Ma::Tree>(ptr(a.root, true), f));
...
//  template<class Ma, class F>
//  static  M map_filter(const Ma& a, const F& f, size_t granularity=kNodeLimit) {
...
struct map_full_entry : entry {
  ...
  static inline const val_t& get_val(const entry_t& e) { return std::get<1>(e); }
...
template <class _Entry, size_t BlockSize=128, class Balance=weight_balanced_tree>
using diff_encoded_map = pam_map<_Entry, BlockSize, diffencoded_entry_encoder, Balance>;
...
template <class _Entry, size_t BlockSize=128, class Balance=weight_balanced_tree>
using diff_encoded_set = pam_set<_Entry, BlockSize, diffencoded_entry_encoder, Balance>;
...
template <typename data, size_t BlockSize=256, class Encoder=default_entry_encoder, class Balance = weight_balanced_tree>
using pam_seq =
